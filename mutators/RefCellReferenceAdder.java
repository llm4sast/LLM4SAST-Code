...
public class RefCellReferenceAdder extends Mutator {
    @Override
    public HashMap<int[], String> searchCandidates(TSNode node, String srcCode) {
        ...
        if (node.getType().equals("function_item")) {
            // RefCell不能在const函数中用
            if (isConstFunction(node, srcCode)) {
                return edits;
            }
                    ...
                    if (param.getType().equals("parameter")) {
                        ...
                        if (!type.isNull() && !type.getType().equals("reference_type") && !pattern.isNull()) {
                            ...
                            String derefAssignment = isMut ?
                                    "\n    let mut " + paramName + " = **" + paramName + ".borrow_mut();" :
                                    "\n    let " + paramName + " = **" + paramName + ".borrow();";
                            int insertPosition = body.getStartByte() + 1;
                            newCode.insert(insertPosition, derefAssignment);

                            // 添加引用
                            String newParamCode = isMut ?
                                    paramCode.replace(typeCode, "std::cell::RefCell<&mut " + typeCode + ">") :
                                    paramCode.replace(typeCode, "std::cell::RefCell<&" + typeCode + ">");
                            ...
                            for (int j = callSites.size() - 1; j >= 0; j--) {
                                ...
                                String callCode = newCode.substring(callSite[0], callSite[1]);
                                ...
                                if (!arguments.isNull()) {
                                    ...
                                    newCode.replace(arg.getStartByte(), arg.getEndByte(),
                                            isMut ?
                                                    "std::cell::RefCell::from(&mut (" + argCode + "))" :
                                                    "std::cell::RefCell::from(&(" + argCode + "))");
                                }
