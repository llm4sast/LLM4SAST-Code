...
class InvokeDynamic {
  ...
  void FN_npeViaCaptureBad(List<String> list) {
    ...
    Collections.sort(
        list,
        (String a, String b) -> { return s.compareTo(a); });
  }
  ...
  static class A { int val; }
  static class Fun1 {
    Function<A, A> f1;
    Fun1(Function<A, A> f1) { this.f1 = f1; }
  }
  static class Fun2 {
    Fun1 f2;
    Fun2(Fun1 f2) { this.f2 = f2; }
  }
  static class Box {
    A a;
    public Box(A a) { this.a = a; }
    public Box map(Function<A, A> f) { return new Box(f.apply(this.a)); }
    public Box mapWithFun(Fun2 fun) { return new Box(fun.f2.f1.apply(this.a)); }
  }
