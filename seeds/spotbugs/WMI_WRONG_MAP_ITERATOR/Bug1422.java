...
public class Bug1422 {
    ...
    public Bug1422(boolean b) {
        ...
        if(b) {
            map2 = new HashMap<>();
        } else { map2 = new EnumMap<>(TimeUnit.class); }
    }
    @NoWarning("WMI_WRONG_MAP_ITERATOR")
    public void iterateEnumMap(EnumMap<TimeUnit, String> map) {
        for(TimeUnit u : map.keySet()) { System.out.println(u+": "+map.get(u)); }
    }
    ...
    public void iterateEnumMapField() {
        for(TimeUnit u : map1.keySet()) { System.out.println(u+": "+map1.get(u)); }
    }
    @ExpectWarning("WMI_WRONG_MAP_ITERATOR")
    public void iterateMapField() {
        for(TimeUnit u : map2.keySet()) { System.out.println(u+": "+map2.get(u)); }
    }
}