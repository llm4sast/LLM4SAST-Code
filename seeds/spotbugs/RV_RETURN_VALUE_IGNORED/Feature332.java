...
public class Feature332 {
    ...
    public void sizeUsed() {
        ...
        if(Math.random() > 0.5) { test.add("qq"); }
        for(int p = test.size(); p > 0; p--) { System.out.println("qq"); }
    }
    @NoWarning("UC")
    public String testStringBuilder(Object obj) { return obj == null ? null : "test" + obj; }
    @NoWarning("UC")
    public void testVarArgs(String f) { String.format(f, 0); }
    @NoWarning("UC")
    public void testUseThrower() throws Exception { raiseException("test", "best", "quest"); }
    public void raiseException(String... message) throws Exception { throw new Exception(Arrays.asList(message).toString()); }
}