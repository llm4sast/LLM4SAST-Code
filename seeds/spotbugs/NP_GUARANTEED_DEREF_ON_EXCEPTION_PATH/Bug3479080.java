...
public class Bug3479080 {
    ...
    public Object doit() {
        ...
        catch (IllegalAccessException iae) { throw new RuntimeException("Cannot create object of type " + c, iae); }
        catch (InvocationTargetException ite) { throw new RuntimeException(c.getName() + ".<init> failed", ite.getCause()); }
        catch (InstantiationException ie) { throw new RuntimeException("Cannot create " + c.getName(), ie); }
        catch (ClassNotFoundException cnfe) { throw new RuntimeException("Cannot create " + targetClass, cnfe); }
    }
