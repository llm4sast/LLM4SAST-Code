...
public class Feature326 {
    ...
    public void testTwoSlotParameters(long l1, int m2, long l3) {
        if(m2 == 199 || m2 == 200) { System.out.println("test"); }
        if(l3 == 199) { System.out.println("test"); }
        if(l1 == 199) { System.out.println("test"); }
    ...
    public void testInverseCondition(int p) throws Exception {
        if(4<p) {
            throw new Exception();
        } else if(4>p) { System.out.println("!!!"); }
        System.out.println("???");
    }
    @ExpectWarning("UC_USELESS_CONDITION")
    ...
    public int testUselessInCatch(boolean c, String d) {
        try {
            if(c) { Integer.parseInt(d); }
        }
        catch(NumberFormatException ex) {
            if(!c) { System.out.println("useless"); }
        }
        return 0;
    }
    @NoWarning("UC_USELESS_CONDITION")
    ...
    public int testLongMax(long l) {
        if(l > 0 && l <= Long.MAX_VALUE) { return 1; }
        return 0;
    }
    @ExpectWarning("UC_USELESS_CONDITION_TYPE")
    public int testLongMin(long l) {
        if(l < 0 && l >= Long.MIN_VALUE) { return 1; }
        return 0;
    }
    @ExpectWarning("UC_USELESS_CONDITION_TYPE")
    ...
    public int testIntMax(int i) {
        if(i > 0 && i <= Integer.MAX_VALUE) { return 1; }
        return 0;
    }
    @NoWarning("UC_USELESS_CONDITION_TYPE")
    ...
    public void testLoop2() {
        for(int i=0; i<10; i++) {
            i++;
            if(i < 3) {
                if(i > 5) { System.out.println("i < 10"); }
                i++;
            }
        }
    }
    ...
    public void testReuseSlotOk(boolean b, int i1, int i2) {
        if(b) {
            int a = i1;
            if(a > 0) { System.out.println("i1 > 0"); }
        } else {
            int a = i2;
            if(a <= 0) { System.out.println("i2 <= 0"); }
        }
    }
    @ExpectWarning("UC_USELESS_CONDITION")
    ...
    public void testNextChar(int i) {
        if (current != -1) {
            while (current != ']' && current != -1) { current = i; }
            if (current != -1) { return; }
        }
    }
    @NoWarning("UC_USELESS_CONDITION")
    public void testNextChar2() throws IOException {
        if (current != -1) {
            while (current != ']' && current != -1) { update(); }
            if (current != -1) { return; }
        }
    }
    private void update() throws IOException { current = System.in.read(); }
}