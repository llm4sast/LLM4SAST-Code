...
class A {
  void method() {
    ...
    throw new MyCheckedException(); // Noncompliant {{Remove the usage of the checked exception
    // 'MyCheckedException'.}}
    //        ^^^^^^^^^^^^^^^^^^^^^^^^
    throw new IOException(); // Noncompliant {{Remove the usage of the checked exception
    // 'IOException'.}}
