class A {
  ...
  public void doTheThing() {
    ...
    double result =
        divide(
            dividend,
            divisor); // Noncompliant {{Parameters to divide have the same names but not the same
  ...
  void test(int parameter1, int parameter2) {
    ...
    // in the cases below we expect them not to raise an issue, because java.util.Objects.equals is
...
class B {
  ...
  void foo2(int a, int d) {
    ...
    strange(
        new B().upperBound,
        Integer.valueOf(0),
        null); // Compliant if only 1 argument matches a parameter in a different position
  }
}
