class Foo {
  void foo() {
    "foo"
        .equals(
            "bar"); // Noncompliant {{Move the "bar" string literal on the left side of this string
    // comparison.}}
    foo.equals(
        "qux"); // Noncompliant [[sc=16;ec=21]] {{Move the "qux" string literal on the left side of
    ...
    "foo"
        .equalsIgnoreCase(
            ""); // Noncompliant {{Move the "" string literal on the left side of this string
    // comparison.}}
    StringUtils.equals("", "");

    foo().bar().equals(""); // Noncompliant
  }
}
...
class QuickFixes {
  void foo() {
    ...
    "foo".equals("probably too long to put in context menu"); // Noncompliant
    // [[sc=18;ec=60;quickfixes=qf2]]
    ...
    foo().bar().equals(""); // Noncompliant [[sc=6;ec=8;quickfixes=qf4]]
  ...
  boolean doesNotHaveWings(String s, boolean isMammal, boolean exp) {
    return (isMammal && s != null && s.equals("bat"))
        ? false
        : exp; // Noncompliant [[sc=47;ec=52;quickfixes=qf5]]
