...
public class MyTest {
  ...
  static {
    try {
      ...
      Assert.fail(
          e.getMessage()); // Noncompliant {{Remove this failure assertion and simply add the
      // exception type to the method signature.}}
      //    ^^^^^^^^^^^^^^^^^^^^^^^^^^^
    }
  ...
  public void testMethodKoWithMessage() {
    ...
    try {
      ...
      org.fest.assertions.Fail
          .fail(); // Noncompliant {{Remove this failure assertion and simply add the exception type
