...
abstract class Class {
  ...
  @Annotation4(
      key = 1 & 2 | 3) // Noncompliant {{Add parentheses to make the operator precedence explicit.}}
  //                   ^^^^^
  public int method(int[] array, int value) {
    ;
    // should raise an issue in initializer
    int b =
        1 & 2 | 3 & 4; // Noncompliant {{Add parentheses to make the operator precedence explicit.}}

    // should raise an issue in operand of unary / parenthesized expression
    // nested kind should not leak outside
    b =
        +(1 & 2 | 3)
            + 2; // Noncompliant {{Add parentheses to make the operator precedence explicit.}}
    ...
    b =
        method(array, 1 & 2 | 3)
            + 1; // Noncompliant {{Add parentheses to make the operator precedence explicit.}}

    int[][] c = {
      {1 & 2 | 3, 2 | 3, 3}, {1 << 2, 1 + 2}
    }; // Noncompliant {{Add parentheses to make the operator precedence explicit.}}
    ...
    do {
      b = 1 & 2 | 3; // Noncompliant {{Add parentheses to make the operator precedence explicit.}}
        ...
        > 42); // Noncompliant {{Add parentheses to make the operator precedence explicit.}}
    ...
    for (int j = 0;
        ...
        j =
            1 & 2
                | 3) { // Noncompliant {{Add parentheses to make the operator precedence explicit.}}
    }
    for (; ; ) {
    ...
    Predicate<Object> p6 =
        (value > 0)
            ...
            : x ->
                x == null
                    ? true
                    : false; // Noncompliant {{Add parentheses to make the operator precedence
    // explicit.}}
