class A {
  void foo() {
    if (something) { // Noncompliant {{Replace this if-then-else statement by a single return
      // statement.}}
      //  ^^
      return true;
    } else {
...
abstract class B {
  ...
  void qix3() {
    if (bar()) { // Noncompliant {{Replace this if-then-else statement by a single method
      // invocation.}}
      bar(foo(), "foo", true);
    } else {
      bar(foo(), "foo", false);
  ...
  boolean qix5() {
    if (foo() == null) { // Noncompliant {{Replace this if-then-else statement by a single method
      // invocation.}}
  ...
  void qix12(Object o) {
    if (bar()) // Compliant
    bar(o, "foo", true);
    else foo(o, "foo", false);
  }

  void qix12(Object o) {
    if (bar()) // Compliant
    bar(o, "foo", bar());
    else bar(o, "foo", bar(o, "foo"));
  }
  ...
  boolean qix14(Object o) {
    if (bar()) // Compliant
    return true;
    else return bar(o, "foo", false);
  }

  boolean qix15(Object o) {
    if (bar()) // Noncompliant
    bar(o, false, "foo", true);
    else bar(o, true, "foo", false);
  }
