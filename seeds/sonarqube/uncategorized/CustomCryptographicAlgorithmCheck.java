...
class A { // Compliant
  void foo() {
    MessageDigest md =
        new MessageDigest() { // Noncompliant {{Make sure using a non-standard cryptographic
          // algorithm is safe here.}}
          //                         ^^^^^^^^^^^^^
...
abstract class B
    extends MessageDigest { // Noncompliant {{Make sure using a non-standard cryptographic algorithm
  // is safe here.}}
  //             ^
