...
public class CheckMyConstraint implements ConstraintValidator<MyConstraint, Department> {
  ...
  public boolean isValid(
      ...
      ConstraintValidatorContext
          context) { // Compliant - @Valid inside constraint validators would be nonsense
  }
}
...
class NonCompliantGroup {
  ...
  private List<User>
      members; // Noncompliant {{Add missing "@Valid" on "members" to validate it with "Bean
  // Validation".}}
  //        ^^^^^^^^^^
...
class CompliantGroup {
  @Valid private Department department; // Compliant

  @Valid private User owner; // Compliant

  @Valid private List<User> members; // Compliant
}
...
class NonCompliantService {
  public void login(
      User user) { // Noncompliant {{Add missing "@Valid" on "user" to validate it with "Bean
    // Validation".}}
  }

  public List<User> list(
      Department
          department) { // Noncompliant {{Add missing "@Valid" on "department" to validate it with
    // "Bean Validation".}}
    //                       ^^^^^^^^^^
  }
}
...
class CompliantService {
  ...
  private void updateLastLoginDate(
      User user) { // Compliant - Bean Validation does not intercept private methods
  }
