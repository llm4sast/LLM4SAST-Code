class A {
  ...
  public boolean isFruitGrape(Fruit candidateFruit) {
    ...
    //                 ^^^^^^
    if (equals(new A())) {}
    return candidateFruit.equals(
        Fruit.GRAPE); // Noncompliant {{Use "==" to perform this enum comparison instead of using
    // "equals"}}
  }
  ...
  public boolean isFruitGrape(Fruit candidateFruit) {
    return candidateFruit
        == Fruit
            .GRAPE; // Compliant; there is only one instance of Fruit.GRAPE - if candidateFruit is a
    // GRAPE it will have the same reference as Fruit.GRAPE
  }
