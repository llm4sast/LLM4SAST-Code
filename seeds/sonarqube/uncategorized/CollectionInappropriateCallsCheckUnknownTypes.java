...
class CollectionInappropriateCallsCheck {
  private void myMethod() {
    ...
    myUnknownCollection.stream()
        .filter(s -> myASet.contains(s.toString()))
        .collect(Collectors.toSet()); // Compliant
  ...
  public String testSplitArray() {
    ...
    String test =
        myGrades.stream()
            ...
            .filter(grade -> badGrades.contains(grade))
            . // compliant, type inference is unable to resolve type of grade because of
            // unknownMethod
            findFirst()
            .orElse("Nothing");
  }
