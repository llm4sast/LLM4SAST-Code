...
class A {
  ThreadLocal<List<String>> myThreadLocal =
      new ThreadLocal<
          List<String>>() { // Noncompliant {{Replace this anonymous class with a call to
        // "ThreadLocal.withInitial". (sonar.java.source not set. Assuming 8 or
        // greater.)}}
        //      ^^^^^^^^^^^^^^^^^^^^^^^^^
        @Override
  ...
  ThreadLocal<List<String>> myThreadLocal6 = new ThreadLocal<List<String>>() { // compliant
        final String myVar = "";
        final String myVar2 = "";
      };
